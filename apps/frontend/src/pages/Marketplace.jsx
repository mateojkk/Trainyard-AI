...
export default function Marketplace() {
  ...
  useEffect(() => {
    fetchListings();
  }, [selectedCategory, debouncedQuery, page, account]);

  const totalPages = Math.ceil(total / 12) || 1;

  return (
    <div className="space-y-8">
      <MarketplaceHero account={account} onLogin={login} />
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-[#0d0d0d] p-3 border border-[#1a1a1a] rounded">
            <div className="relative w-full md:max-w-xs">
              ...
              <input
                ...
                className="block w-full pl-9 pr-3 py-1.5 bg-[#141414] border border-[#2e2e2e] focus:border-brand-blue rounded text-xs text-gray-200 placeholder-gray-500 focus:outline-none transition duration-150 font-sans"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-1 w-full md:w-auto">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs font-semibold cursor-pointer transition duration-150 ${selectedCategory === cat.id ? "bg-[#38bdf8]/10 text-brand-blue border border-brand-blue/30" : "bg-transparent text-gray-400 hover:text-gray-200 border border-transparent"}`}
                >
                  {cat.label}
                </button>
              ))}
            ...
            <>
              ...
              <div className="flex items-center justify-between border-t border-[#1a1a1a] pt-4 mt-8">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1 border border-[#2e2e2e] hover:border-brand-blue rounded text-xs text-gray-400 hover:text-gray-200 disabled:opacity-30 disabled:hover:border-[#2e2e2e] transition cursor-pointer">Previous</button>
                  <div className="flex items-center text-xs font-mono text-gray-400 px-2">Page {page} of {totalPages}</div>
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1 border border-[#2e2e2e] hover:border-brand-blue rounded text-xs text-gray-400 hover:text-gray-200 disabled:opacity-30 disabled:hover:border-[#2e2e2e] transition cursor-pointer">Next</button>
                </div>
              </div>
            </>
          )}
