...
export default function Landing({ onLogin, authError }) {
  ...
  useEffect(() => {
    ...
    link.rel = 'preload'; link.as = 'image'; link.href = landingBackground;
  ...
  if (!bgLoaded) return <section className="w-full min-h-screen sm:h-screen bg-[#1e90ff]" />;

  return (
    <>
      <section className="relative w-full min-h-screen sm:h-screen overflow-hidden bg-[#1e90ff]">
        <div
          className="landing-bg absolute -inset-8 bg-cover bg-center"
          style={{ backgroundImage: `url(${landingBackground})` }}
        />
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-b from-[#D89F55]/20 via-transparent to-[#1e90ff]/25" />
          <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-[#1e90ff]/65 to-transparent" />
          <nav className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between px-4 sm:px-6 md:px-10 py-3 sm:py-6">
            ...
            <div>
              <button onClick={onLogin} className="bg-[#1e90ff] hover:bg-[#3F2420] text-white text-xs sm:text-sm font-medium px-4 sm:px-5 py-2 sm:py-2.5 rounded-full transition-colors cursor-pointer border-0 shadow-sm font-sans">
                Access App
              </button>
            </div>
          </nav>
          <div className="relative z-10 flex flex-col items-center text-center pt-24 sm:pt-28 md:pt-32 px-4 sm:px-6">
            <h1
              className="font-normal leading-[0.95] text-[#1e90ff] text-xl xs:text-2xl sm:text-4xl md:text-5xl lg:text-[4.75rem] xl:text-[5.25rem] max-w-5xl"
          ...
          <div className="absolute left-4 right-4 sm:right-auto sm:left-6 md:left-10 bottom-4 sm:bottom-8 md:bottom-10 z-10 max-w-sm">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <button onClick={onLogin} className="bg-[#1e90ff] hover:bg-[#3F2420] text-white text-sm font-semibold px-5 sm:px-6 py-2.5 sm:py-3 rounded-full transition-colors shadow-sm cursor-pointer border-0">
                Connect via zkLogin
              </button>
            ...
            {authError && (
              <p className="mt-4 rounded-md bg-[#1e90ff]/85 px-3 py-2 text-xs leading-relaxed text-[#D89F55] shadow-sm">
                {authError}
              </p>
            )}
          </div>
          <div className="hidden sm:flex absolute right-6 md:right-10 bottom-8 md:bottom-10 z-10 items-center gap-2 text-white text-sm drop-shadow">
            <button onClick={onLogin} className="flex items-center justify-center w-6 h-6 rounded-full bg-[#1e90ff] hover:bg-[#3F2420] transition-colors cursor-pointer border-0">
